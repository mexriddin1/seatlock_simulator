...
class HomePageBloc extends Bloc<HomePageEvent, HomePageState> {
  ...
  Future<void> _loadSeatsEvent(
    ...
    if (response.isSuccess) {
      ...
      // Start background simulator to simulate concurrent user actions
      if (!_backgroundSimulator.isRunning) {
        _botLogSubscription?.cancel();
        _botLogSubscription = _backgroundSimulator.logStream.listen(
          (log) {
            add(BotLogEvent(log));
          },
        );
  ...
  Future<void> _lockSeatEvent(
    ...
    if (response.isSuccess) {
      ...
      if (data != null) {
        final seats = (state.expirationSeatStatus.dataOrNull ?? [])..add(data);
